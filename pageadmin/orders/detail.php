...
if (isset($_POST['process'])) {
   ...
   $update_orders->execute(['order in process', $invoice]);
   header('location:index.php');
}
if (isset($_POST['completed'])) {

   $update_orders = $conn->prepare("UPDATE `orders` SET status = ? WHERE invoice = ?");
   $update_orders->execute(['completed', $invoice]);
   header('location:index.php');
}
?>
...
<head>
   ...
   <link rel="stylesheet" href="../../products/css/detail_orders.css">
         ...
         ?>
                     ...
                     <div class="box">
                        <div class="col">
                           ...
                           <p class="grand-total">grand total : <span>Rp<?= number_format($grand_total, 0, ',', '.'); ?></span></p>
                        ...
                        <div class="col">
                           ...
                           <p class="user"><i class="fas fa-map-marker-alt"></i><?= $fetch_order['address_table']; ?></p>
                           ...
                           <?php if ($fetch_order['status'] ==  'needs payment') { ?>
                              <form action="" method="POST">
                                 <input type="submit" value="Process" name="process" class="process-btn" onclick="return confirm('process this order?');">
                              </form>
                           <?php } else if ($fetch_order['status'] == 'order in process') { ?>
                              <form action="" method="POST">
                                 <input type="submit" value="completed" name="completed" class="process-btn" onclick="return confirm('completed this order?');">
                              </form>
                           <?php } ?>
                        </div>
                     </div>
